...
4. **Nessun dato censuario** (superficie catastale, qualità, classe, redditi) né intestatari: sono disponibili solo tramite visura. La superficie mostrata è calcolata dalla geometria WFS ed è indicativa. Non esistono open data né API pubbliche con questi campi per singola particella: la scheda offre i collegamenti ai servizi ufficiali (Consultazione rendite catastali gratuita, Visura catastale online, «Consulta i dati dei tuoi immobili» con SPID/CIE) dove completarli a mano, e il pulsante «Copia dati catastali» per incollare i riferimenti. Le righe **Indirizzo (OpenStreetMap)** e **Quota s.l.m.** (Open-Meteo Elevation, DEM 90 m) vengono compilate da fonti aperte e sono indicative.
